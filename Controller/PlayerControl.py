...
def change_one_item (choice,current_list):
        ...
        new_id=""
        list_id=PlayerModel.get_all_id("")
        ...
        while new_id in list_id or Player.check_format_id(new_id)==False:
            ...
            if PlayerModel.check_format_id(new_id)==False:
                PlayerView.display_text("Mauvais format, il faut  ce format : AB1234")
        current_list[choice].id_number=new_id

        new_birthdate=""
        while PlayerModel.check_format_birthdate(new_birthdate)==False:
        ...
        while save_or_not not in ["o","n"]:
            ...
            if save_or_not=="o":
                PlayerModel.change_specific_data(choice,"players_database.json", "id_number", new_id)
                PlayerModel.change_specific_data(choice,"players_database.json", "firstname", new_firstname)
                PlayerModel.change_specific_data(choice,"players_database.json", "surname", new_surname)
                PlayerModel.change_specific_data(choice,"players_database.json", "birthdate", new_birthdate)
                return
            else:
                return
            #if new_data == "":
    ...
    def add_player(current_list):
        ...
        PlayerView.display_text("Vous ajoutez un joueur numéro "+str(index_new_player))
        current_list.append(Player(id_number="",surname="",firstname="",birthdate=""))
        PlayerView.display_text("la longueur de la liste est désormais" + str(len(current_list)))
        change_one_item (index_new_player,current_list)
    ...
    def check_input_choice_player(max_digit):
        ...
        # Vérifier si l'entrée est 'x'
            if choice == 'x':
                PlayerView.display_text("Vous avez choisi d'ajouter un joueur'.")
                return choice
        ...
        # Vérifier si l'entrée est un chiffre entre 1 et 10
            if choice.isdigit():
                ...
                if 0 <= nombre <= max_digit:
                    ...
                    PlayerView.display_text(f"Erreur : Le chiffre doit être entre 0 et {max_digit-1}.")
            else:
                PlayerView.display_text(f"Erreur : Veuillez entrer un chiffre entre 0 et {max_digit-1} ou 'x' pour ajouter un joueur.")
    ...
    def change_player():
       ...
       #                  current_list.birthdate]
        PlayerView.display_text ("Entrez le numéro du joueur pour le changer ou X pour en ajouter.")
        choice=Player.check_input_choice_player(max_digit=len(current_list))
        if choice.isdigit():
            PlayerView.display_text(f"\nVotre choix c'est {current_list[int(choice)].firstname}")
            change_one_item (choice,current_list)
        if choice=="x":
            Player.add_player
