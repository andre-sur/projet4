...
class TournamentModel:
    ...
    def list_of_participants(self):
        ...
        for player in self.players:
            list_participants += Player.find_name_with_code(
                what_code=player)+"\n"
    ...
    def get_matches_display(self):
        ...
        for match in all_matches:
            view_matches += f"\n   ROUND # {counter_round}\n"
            for elements in match:
                view_matches += f"{elements[0][0]} vs {elements[1][0]}"
                " "*(25 - len(f"{elements[0][0]} vs {elements[1][0]}"))
                " : "+f"{str(elements[0][1])} - {str(elements[1][1])}\n"
